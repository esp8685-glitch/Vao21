...
bool waitingForDetectorLine = false;
    void onNew() override
    {
...
bool parseDetectorLine(const String &line)
{
    if (!waitingForDetectorLine)
        return false;
    ...
    addOrUpdateDetector(detectorAddr, currentTimestamp);
    ...
    waitingForDetectorLine = false;

    return true;
}

bool parseAlarmHeader(const String &line)
{
    if (line.indexOf("SUITSUHAIRE") < 0)
        return false;

    currentEventType = "SUITSUHAIRE";

    int smokePos = line.indexOf("SUITSUHAIRE");

    if (smokePos > 0)
    {
        currentTimestamp = line.substring(0, smokePos);
        currentTimestamp.trim();
    }

    waitingForDetectorLine = true;

    return true;
}
bool isEventHeader(const String &line)
{
...
logInfo("USB RX: " + line);
parseAlarmHeader(line);     // just detect alarm and save timestamp
parseDetectorLine(line);    // just detect detector line
    ...
    // String detectorAddr;
    // bool bfirst=tryExtractDetectorAddress(line, detectorAddr);
    // // currentEventIsSmokeAlarm=line.indexOf("SUITSUHAIRE") >= 0;
    // logInfo(String(bfirst) + "-" + String(currentEventIsSmokeAlarm));
    // if (bfirst && currentEventIsSmokeAlarm)
    // {
    //     currentEventIsSmokeAlarm=false;
